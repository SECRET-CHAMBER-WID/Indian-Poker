...
export function Button({ variant = 'soft', size = 'md', icon, className = '', children, ...props }: ButtonProps) {
  return (
    <button
      className={`inline-flex items-center justify-center gap-2 rounded-2xl font-semibold transition duration-150 disabled:cursor-not-allowed disabled:opacity-50 active:shadow-neu-inset ${variantClasses[variant]} ${sizeClasses[size]} ${
